...
from google_research.frechet_audio_distance.audioset_model import AudioSetModel
...
class BatchedInference(beam.DoFn):
  ...
  def _flush_buffer(self, add_window=False):
    ...
    for key, example in self._buffer:
      ...
      features_all = self._model.extract_features(dist_samples)
...
class ReadWavFiles(beam.DoFn):
  ...
  def process(self, element):
    ...
    data = np.pad(data, (0, 16000 - len(data)%16000), 'constant')
