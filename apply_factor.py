...
if __name__ == "__main__":
    ...
    with dnnlib.util.open_url(args.ckpt) as f:
        G = legacy.load_network_pkl(f, **G_kwargs)['G_ema'].to(device) # type: ignore
    ...
    for l in latents:
        print(f"Generate audios for seed ", l)
        ...
        if len(index) ==  1 and index[0] == -1: # use all eigenvalues
            ...
            file_name_base = f"seed-{l}_index-all_degree-{args.degree}.wav"
        else: # use certain indexes as eigenvalues
            index_list_of_eigenvalues = index
            str_index_list = '-'.join(str(x) for x in index)
            file_name_base = f"seed-{l}_index-{str_index_list}_degree-{args.degree}"

        for j in index_list_of_eigenvalues:
            file_name = file_name_base + "eigidx" + str(j)

            current_eigvec = eigvec[:, j].unsqueeze(0)
            direction = args.degree * current_eigvec
            generate_audios(z, label, truncation_psi, noise_mode, direction, args.output, file_name)
#            audio_grid_eigvec.append(image_group)
